...
async function stitchAndUpload(clipUrls, bookId, title, authorName, narrationTracks = [], musicAudioPath = null, characterLines = [], trailerId = null) {
  ...
  const storagePath = trailerId
    ? `trailers/${bookId}/${trailerId}.mp4`
    : `trailers/${bookId}/final-trailer.mp4`
...
async function runPipeline(job) {
  const { bookId, trailerId, tier, quality = 'standard', narratorVoice: requestedNarratorVoice = null } = job
  ...
  // Pick narrator voice: use author's explicit choice if provided, otherwise auto-pick
  // based on book genre + character genders.
  const narratorVoice = requestedNarratorVoice || pickNarratorVoice(book.genre, characters)
  const voiceSource = requestedNarratorVoice ? 'author choice' : 'auto (genre/gender)'
  console.log(`[worker]   Narrator voice: ${narratorVoice} (${voiceSource}, genre: ${book.genre || 'unknown'})`)
  ...
  for (const scene of scenesToGenerate) {
    try {
      ...
      if (sceneErr.isModeration || sceneErr.isBadOutput) {
        ...
        // Non-moderation failure (transient EvoLink/network error, billing issue, etc.)
        // Use 'pending' so it auto-retries on next run instead of getting stuck
        await supabase.from('scenes').update({
          moderation_status: 'pending',
          moderation_reason: null,
          last_moderation_at: new Date().toISOString(),
